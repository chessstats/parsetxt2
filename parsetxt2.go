				...
				if err==nil {
					...
					if invalid {
						...
						if regexp.MustCompile("aaberg, anton").MatchString(head) {
							headm=regexp.MustCompile(`^ 1701991 `).ReplaceAllString(headm,"id_number")
							headm=regexp.MustCompile(`aaberg, anton`).ReplaceAllString(headm,"name         ")
							headm=regexp.MustCompile(`     swe`).ReplaceAllString(headm,"tit  fed")
							headm=regexp.MustCompile(`[0-9]{4}`).ReplaceAllString(headm,"rtg ")
							headm=regexp.MustCompile(`17.01.72`).ReplaceAllString(headm,"born    flag        ")
							headm=regexp.MustCompile(` 0 `).ReplaceAllString(headm,"gms")
							txtfile,err=os.Open(root+string(os.PathSeparator)+name)
						} else {
							//invalid=false
						}

						if regexp.MustCompile("apr03").MatchString(name) {
							headm="id_num   name                             tit  fed  rtg    gms born     flag   "
						}
						if regexp.MustCompile("apr04").MatchString(name)||regexp.MustCompile("jan05").MatchString(name)||regexp.MustCompile("jul04").MatchString(name)||regexp.MustCompile("jul05").MatchString(name)||regexp.MustCompile("oct02").MatchString(name)||regexp.MustCompile("oct04").MatchString(name) {
							headm="id_num    name                             tit  fed  rtg    gms born     flag   "
						}
						if regexp.MustCompile("jan06").MatchString(name) {
							headm="id_num    name                             tit  fed  rtg    gms born flag"
							skipfirst=true
						}
						if regexp.MustCompile("jul11").MatchString(name) {
							headm="id_num    name                             tit  fed  rtg    gms born  flag        "
						}
						if regexp.MustCompile("jan03").MatchString(name) {
							headm="id_num    name                             tit  fed  rtg    gms born       flag        "
						}
